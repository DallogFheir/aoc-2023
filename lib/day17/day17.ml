...
let part_1_aux path =
  ...
  let target_coords = (col_length - 1, row_length - 1)
    ...
    if PriorityMap.is_empty queue then ()
      ...
      then find_shortest_path (PriorityMap.remove node queue)
        ...
        Hashtbl.replace visited
          ...
          (path_length, prev) ;
        get_neighbors coords count_in_straight_line from_direction grid
        |> List.fold_left
             (fun new_queue
  ...
  let _, (path_length, _) =
    ...
    |> List.find_all (fun ((coords, _, _), _) -> coords = target_coords)
...
let test_1 () =
  part_1_aux "lib/day17/test.txt" |> print_int ;
  print_newline ()

let part_1 () = part_1_aux "lib/day17/input.txt"
(*
   let test_2 () =
     part_2_aux "lib/day17/test.txt" |> print_int ;
     print_newline ()
